...
from TimeBlock.db_timeblock import create_timeblock as db_create_timeblock, get_timeblocks_by_employee, get_timeblocks_by_project, update_timeblock
...
def get_timeblocks_by_employee(employee_id: int, db: Session = Depends(get_db)):
    return get_timeblocks_by_employee(db, employee_id)
...
def get_timeblocks_by_project(project_id: int, db: Session = Depends(get_db)):
    return get_timeblocks_by_project(db, project_id)
...
def update_timeblock(timeblock_id: int, request: TimeBlockCreate, db: Session = Depends(get_db)):
    tb = update_timeblock(db, timeblock_id, request)
    if not tb:
